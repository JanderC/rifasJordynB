...
router.get('/rifas/:rifaId/lotes-vendedores', async (req, res) => {
  ...
  try {
    ...
    // 5. Cargar estado de cuadre para saber si un vendedor ya cerró
    const cuadreR = await client.query(
      `SELECT vendedor_id::text, cuadrado, monto_cuadrado FROM caja_cuadre_vendedor WHERE rifa_id=$1`,
      [rifaId]
    );
    const cuadreMap = {};
    for (const row of cuadreR.rows) {
      cuadreMap[row.vendedor_id] = { cuadrado: row.cuadrado, monto_cuadrado: row.monto_cuadrado };
    }

    // 6. Para cada vendedor: buscar o crear su lote
    const resultado = [];
    for (const v of vendsR.rows) {
      ...
      // Buscar lote por vendedor_id — traer todos los campos necesarios
      let loteR = await client.query(
        `SELECT id, abono, por_pagar, pendiente, estado FROM caja_lotes WHERE semana_id=$1 AND vendedor_id=$2 LIMIT 1`,
        [semanaId, v.vendedor_id]
      );
      // Fallback por nombre (lotes viejos sin vendedor_id)
      if (!loteR.rows[0]) {
        loteR = await client.query(
          `SELECT id, abono, por_pagar, pendiente, estado FROM caja_lotes WHERE semana_id=$1 AND vendedor_nombre=$2 LIMIT 1`,
      ...
      if (loteR.rows[0]) {
        ...
        // Solo recalcular por_pagar si el vendedor NO está cuadrado.
        // Si ya cuadró, respetar el monto_cuadrado guardado para no pisar el cierre.
        const yaCuadrado = cuadreMap[v.vendedor_id]?.cuadrado === true;
        if (!yaCuadrado && porPagar > 0) {
          await client.query(
            `UPDATE caja_lotes SET por_pagar=$1, pendiente=GREATEST($1 - COALESCE(abono,0), 0) WHERE id=$2`,
            [porPagar, lote.id]
          );
          // Reflejar los valores actualizados en el objeto local
    ...
    await client.query('COMMIT');
    res.json({ semana_id: semanaId, lotes: resultado, porcentaje, precio_con_pct: precioConPct });
  } catch (e) {
    await client.query('ROLLBACK');
