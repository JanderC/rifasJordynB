  ...
  try {
    ...
    if (password) {
      if (password.length < 6)
        return res
          .status(400)
          .json({ error: "Contraseña mínimo 6 caracteres" });
  ...
  try {
    ...
    if (parseInt(ventas.rows[0].count) > 0)
      return res.status(400).json({
        error:
          "No se puede eliminar un vendedor con ventas registradas. Desactívalo en su lugar.",
      });
    await pool.query(
      "DELETE FROM numeros_vendedor_global WHERE vendedor_id = $1",
      [req.params.id],
    );
    await pool.query(
      "DELETE FROM numeros_vendedor        WHERE vendedor_id = $1",
      [req.params.id],
    );
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const excluirFinal = [
        ...new Set([...propiosR.rows.map((r) => r.numero), ...excluirExtra]),
      ];
      ...
      const result = await pool.query(
       ...
       ORDER BY RANDOM() LIMIT $1`,
        excluirFinal.length > 0
          ? [parseInt(cantidad), excluirFinal]
          : [parseInt(cantidad)],
      );
...
numRouter.post("/asignar", authMiddleware, soloDueno, async (req, res) => {
  ...
  if (!vendedor_id || !Array.isArray(numeros) || !numeros.length)
    return res
      .status(400)
      .json({ error: "vendedor_id y numeros[] son requeridos" });

  const invalidos = numeros.filter((n) => !/^\d{3}$/.test(n));
  if (invalidos.length)
    return res
      .status(400)
      .json({ error: `Números inválidos: ${invalidos.join(", ")}` });
...
catRouter.post("/", authMiddleware, soloDueno, async (req, res) => {
  ...
  if (!["parcial", "simultanea"].includes(tipo))
    return res
      .status(400)
      .json({ error: 'Tipo debe ser "parcial" o "simultanea"' });
...
catRouter.delete("/:id", authMiddleware, soloDueno, async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Categoría eliminada. Las asignaciones se removieron automáticamente.",
    });
  } catch (err) {
  ...
  async (req, res) => {
    try {
      const catR = await pool.query(
        "SELECT id, tipo FROM categorias_globales WHERE id = $1",
        [req.params.id],
      );
      if (!catR.rows[0])
        return res.status(404).json({ error: "Categoría no encontrada" });
      ...
      for (const num of numerosVendedor) {
        if (tipo === "parcial") {
          ...
          if (!ocupA) {
            ...
            libres.push({ numero: num, serie: "B" }); // ← 002 con Osman en A → cae aquí correctamente
          } else {
            colisiones.push({
              numero: num,
...
catRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        tipo === "simultanea" &&
        seriesActuales.includes("A") &&
        seriesActuales.includes("B")
      ) {
        await client.query("ROLLBACK");
        return res.status(409).json({
      ...
      for (const num of numerosVendedor) {
        if (tipo === "parcial") {
          ...
          if (!ocupA) {
            ...
            aInsertar.push({ numero: num, serie: "B" }); // ← 002 con Osman en A → Serie B
          } else {
            colisiones.push({
              numero: num,
              dueno: `${ocupA} (Serie A) y ${ocupB} (Serie B)`,
            });
          }
        }
      }
      ...
      res.status(201).json({
        message: [
          totalInsertados > 0
            ? `✅ ${totalInsertados} número(s) asignados a ${vendedorNombre}`
            : null,
          colisiones.length
            ? `⚠️ ${colisiones.length} número(s) con conflicto no se asignaron`
            : null,
        ]
          .filter(Boolean)
          .join(". "),
  ...
  async (req, res) => {
    try {
      ...
      if (!result.rows.length)
        return res
          .status(404)
          .json({ error: "Este vendedor no está en la categoría" });
      res.json({
      ...
      console.error(
        "DELETE /categorias-globales/:id/vendedores/:vendedorId:",
        err,
      );
      res.status(500).json({ error: "Error del servidor" });
    }
...
catRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      console.error(
        "DELETE /categorias-globales/:id/asignaciones/:asigId:",
        err,
      );
      res.status(500).json({ error: "Error del servidor" });
    }
