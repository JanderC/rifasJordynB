...
//   Usadas por la página de cliente
// ============================================================
...
/* ──────────────────────────────────────────────────────────
   ...
   Lista rifas activas con imagen, premio, precio, progreso
────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────
   ...
   Devuelve los 1000 números con su estado para el grid
────────────────────────────────────────────────────────── */
router.get('/rifas/:id/numeros-disponibles', async (req, res) => {
  try {
    // Números vendidos confirmados
    const vendidos = await pool.query(`
      SELECT numero, COUNT(*) AS veces
      FROM ventas WHERE rifa_id=$1
      GROUP BY numero
    `, [req.params.id]);

    // Números reservados pendientes de aprobación
...
/* ──────────────────────────────────────────────────────────
   ...
   El cliente escoge número, sube comprobante y sus datos
────────────────────────────────────────────────────────── */
router.post('/reservar', async (req, res) => {
  ...
  try {
    // Verificar que el número no esté agotado
    const vendidos = await pool.query(
    ...
    // Verificar que no haya reserva pendiente del mismo número en la misma rifa
    ...
    `, [rifa_id, numero, nombre_cliente.trim(), telefono || null, metodo_pago || null, comprobante_base64 || null, comprobante_nombre || null]);
...
/* ──────────────────────────────────────────────────────────
   GET /api/publico/reserva/:id  — consultar estado de reserva
────────────────────────────────────────────────────────── */
router.get('/reserva/:id', async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT id, numero, nombre_cliente, estado, nota_admin, created_at, updated_at,
...
/* ══════════════════════════════════════════════════════════
   RUTAS ADMIN — gestión de reservas (solo dueño)
══════════════════════════════════════════════════════════ */

// GET /api/publico/admin/reservas — listar todas
router.get('/admin/reservas', authMiddleware, soloDueno, async (req, res) => {
  const { estado } = req.query;
  try {
    const r = await pool.query(`
      SELECT rc.*, r.nombre AS rifa_nombre, r.precio, r.premio
      FROM reservas_cliente rc
      JOIN rifas r ON r.id = rc.rifa_id
...
// PUT /api/publico/admin/reservas/:id — aprobar o rechazar
router.put('/admin/reservas/:id', authMiddleware, soloDueno, async (req, res) => {
  ...
  try {
    ...
    const res_r = await client.query(
      `UPDATE reservas_cliente SET estado=$1, nota_admin=$2 WHERE id=$3 RETURNING *`,
    ...
    // Si se rechaza → el número queda libre automáticamente
    // (la vista numeros-disponibles filtra solo reservas con estado='pendiente')
    // Si se aprueba → crear venta real
    if (estado === 'aprobado') {
      // Buscar vendedor del sistema como "online" o usar el dueño
